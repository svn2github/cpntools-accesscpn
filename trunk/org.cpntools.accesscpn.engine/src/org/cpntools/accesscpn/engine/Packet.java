...
public class Packet implements Serializable {
	...
	public Packet(final int opcode, final int command) {
		if (opcode != 7 && opcode != 9 && (opcode == 5 && command != 1)) { throw new IllegalArgumentException("Trying to create GPC with wrong opcode"); }
		if (opcode == 5)
			gfcbis = true;
		this.opcode = opcode;
	...
	/**
	 * @param opcode opcode of GPC command
	 * @param parameters GPC parameters
	 */
	...
	/**
	 * @param opcode opcode of eval command (1 or 3)
	...
	private byte[] getByteArray(final DataInputStream in, int length) throws IOException {
	...
	private String getString(final DataInputStream in, int length) throws IOException {
	...
	private int getIntWithPutback(List<Integer> seen, int position, DataInputStream stream) throws IOException {
	...
	public void receive(final DataInputStream in) throws IOException {
		...
		if (isGFC()) {
			...
			// - 7) i3 = 1/2 and #string = 6/8 (or GFC) as #strings = 2 + #trans + 2 * #place = 2 + 2 + 2 * #place = 1, 2, 3, 4, 5, 6, 8 and earlier i3 != 0
								...
								if (getIntWithPutback(seen, 5, in) == 1) { // i1 (command)/p1val
									if (isGFCDirection(getIntWithPutback(seen, 6, in))) { // i2 (delay)/p2dir
										if (isGFCType(getIntWithPutback(seen, 7, in))) { // i3 (places)/p2type
											if (getIntWithPutback(seen, 8, in) == 2) { // i4 (trans)/p2val or p3dir (if p2dir = 2)
												if ((getIntWithPutback(seen, 7, in) == 2 && getIntWithPutback(seen, 2, in) == 8) || (getIntWithPutback(seen, 7, in) == 1 && getIntWithPutback(seen, 2, in) == 6)) {
													if ((getIntWithPutback(seen, 1, in) == 8) || (getIntWithPutback(seen, 1, in) == 10)) {
														gfcbis = true;
													}
												}
											}
		...
		if (isBIS() || (isGFCBIS())) {
			...
			for (int j = 0; j < args; ++j) {
				if (getIntWithPutback(seen, pos++, in) != 1) { throw new IOException("Recieved parameter INOUT or OUT; neither is supported."); }
	...
	private boolean isGFCDirection(int value) {
		return value == 1 ||
		value == 2 ||
		value == 3;
	}

	private boolean isGFCType(int value) {
		return value == 1 ||
		value == 2 ||
		value == 3 ||
		value == 4 ||
